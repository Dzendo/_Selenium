...
class CustomTestExecutionListener : TestExecutionListener {
    ...
    override fun executionStarted(testIdentifier: TestIdentifier) {
        if (DT >7) println("ExecutionListener Started test:  ${testIdentifier.displayName} ${testIdentifier.source} ")
    ...
    override fun executionFinished(testIdentifier: TestIdentifier, testExecutionResult: TestExecutionResult) {
        if (DT >7) println("ExecutionListener Finished test: ${testIdentifier.displayName}  ${testExecutionResult.status}")
        startDialog?.showActionTests(testIdentifier.displayName, testExecutionResult.status.toString())
        if (TestsProperties.isStartStop > 0) { } // как то можно остановить ??? Обнулить тест план??

        if (TestsProperties.isPaused) {
            if (DT >4) println("ExecutionListener Pausing for Resume")
            try {
                while(TestsProperties.isPaused) Thread.sleep(1000L)
            } catch (e: InterruptedException) {
                e.printStackTrace()
            }
            if (DT >4) println("ExecutionListener  Resume")
        }
        if (TestsProperties.isExit) {
            if (DT >4) println("ExecutionListener Exiting test execution")
            exitProcess(0)
        }
    }
}
