...
class LoginPage(driver: WebDriver) {
     ...
     */
    fun inputLogin(login: String) = webDriverWait.until(ExpectedConditions.elementToBeClickable(loginField)).sendKeys(login)

    /**
     * метод для ввода пароля
     */
    fun inputPasswd(passwd: String) = webDriverWait.until(ExpectedConditions.elementToBeClickable(passwdField)).sendKeys(passwd)

    /**
     * метод для осуществления нажатия кнопки входа в аккаунт
     */
    fun clickLoginBtn() = webDriverWait.until(ExpectedConditions.elementToBeClickable(loginBtn)).click()

    /**
     * метод для осуществления нажатия кнопки подтверждения выхода из TDMS
     */
    fun ClickYesBtn() = webDriverWait.until(ExpectedConditions.elementToBeClickable(yesBtn)).click()
}